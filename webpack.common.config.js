...
const GetTargetPath = () => {
    return path.resolve("./dist", process.env.BUILD_ENV);
}
...
const VersionStringReplacer = (dir, files) => {
    const now = new Date();
    const major = `1`;
    const minor = `${Pad(now.getFullYear(), 4)}${Pad(now.getMonth()+1, 2)}${Pad(now.getDate(), 2)}`;
    const patch = Number(`${Pad(now.getSeconds() + (60 * (now.getMinutes() + (60 * now.getHours()))), 5)}`).toString();
